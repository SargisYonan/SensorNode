...
void parse_command(void) {
   ...
   if(strcasestr(parser_buffer, "L?")){
      ...
      int stop = i - 1; 
      for (i = stop; i >= start; i--) tval += (((uint16_t) parser_buffer[i]) - 48) * pow(10, stop - i); // add every digit
      ...
      if (!parser_flags.command_error_setpoint) {
         ...
         parser_flags.var_setpoint = tval < 99 ? tval : tval + 1; // this is some wierd stuff
      }
   }
   else if (strcasestr(parser_buffer, "S?")) {
